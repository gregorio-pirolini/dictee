...
jQuery(document).ready(function ($) {
	...
	let aAskedWords = []; //arr of words asked
	...
	// length of words max 12 letters vanilla js
	// $('#spellwordsBox input').prop("maxLength", 12);
	document.querySelector("#spellwordsBox input").maxLength = 12;
	console.log("aKeys");
	createToy(aKeys); //variables.js
	getUserInfo();

	// is regex ok vanilla js
	function checkInRegex(val) {
		//one letter and in regex
		...
		$.getJSON("php/getUserInfo.php", function (data, status) {
			...
			$("#slider").slider({
				...
				stop: function (event, ui) {
					...
					console.log("stoped ---------------------");
					$.post(
						"php/update_audio.php",
						"audio=" + ui.value + "&id=" + $id,
	...
	function createToy(arr) {
		...
		$.each(arr, function (key, value) {
			...
			myKeyBoard +=
				...
				value[2] + '"'  + addId +
				'><table width=100% height=100%><tr><td valign=middle class="' +
				value[5] +
				'" data-keyCode="' +
				value[1] +
				'">' +
	...
	$("body").keypress(function (e) {
		al3rt("keypress listenToMe "+listenToMe  );
	...
	function startGame() {
		console.log('aAskedWords.length: '+ aAskedWords.length);
	...
	function doGame(game, arr) {
		//'spell',aAskedWords,nb ->jeux array nb
		...
		if (art == "spell") {
			...
			if (番号 < maxRound) {
				...
				if (round % 2 === 0) {
					...
					actualQuestion = [aEpelleLong[randomOrder], aAskedWords[番号]];
				}
	...
	function sendPoints(from) {
		...
		$.each(aAskedWords, function (id, val) {
			...
			$.post(
				"php/update_word_value.php",
				"val=" + val[3] + "&id=" + val[2] + "&userId=" + $id,
				function (result, status) {
	...
	function updateLevel() {
		...
		$.post(
			"php/update_level.php",
			"userId=" + $id + "&levelLetter=" + $level,
			function (result, status) {
				console.log(result);
				...
				if (result.search("Félicitations: tu passes au niveau suivant!") > -1) {
					...
					$.post(
						"php/update_levelUser.php",
						"level=" + aLevels[levels] + "&userid=" + $id,
						function (result, status) {}
					);
				}
	...
	function checkWord(test) {
		...
		if (test == wordAsked) {
			...
			if (tried < 1 && aideWasClicked == false) {
				//first time no help
				pts = 100;
			...
			//word not spelled write
			valueForWord = 4;
			aAskedWords[round - 1][3] = valueForWord;

			var randomFaux = Math.floor(Math.random() * (aFaux.length - 1) + 0);
			if (tried < 1) {
				var randomRecommence = Math.floor(
					Math.random() * (aRecommence.length - 1) + 0
				);
				var randomaEpelleCourt = Math.floor(
					Math.random() * (aEpelleCourt.length - 1) + 0
				);
				aWordsItsWrong = [
					aFaux[randomFaux],
				...
				var randomCorrection = Math.floor(
					Math.random() * (aCorrection.length - 1) + 0
				);
				aCorrectionToSpeak = [
					[aCorrection[randomCorrection][0][0], "ordres/correction"],
					[aCorrection[randomCorrection][0][1], "ordres/correction"],
				];
				aWordsItsWrong = [
					aFaux[randomFaux],
					aCorrectionToSpeak[0],
					aAskedWords[番号],
					aCorrectionToSpeak[1],
				];
	...
	function doUppercase() {
		if (caps == false) {
			$(".letter").css("text-transform", "uppercase").css("font-size", "50px");
			caps = true;
		} else {
			doLowercase();
		}
	...
	function doLowercase() {
		$(".letter").css("text-transform", "lowercase").css("font-size", "55px");
		caps = false;
	}
